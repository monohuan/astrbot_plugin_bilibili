...
## ✨ 功能特性
  ...
  - **UP 主动态订阅**：
      ...
      - 多图动态可选「原图合并转发」：推送卡片后以合并消息附带全部原图，支持会话级开关与管理员全局强制。
...
其他配置项（轮询间隔、图片渲染开关、渲染模板、代理、AI 摘要等）可在 AstrBot 的插件配置面板中调整。其中 `enable_video_image` / `enable_bangumi_image` 分别控制自然语言搜索的视频/番剧列表是否渲染为图片（默认开启）。
...
| 指令 | 参数 | 说明 | 别名 |
| :--- | :--- | :--- | :--- |
| **bili_sub** | `<B站UID> [过滤器...]` | 订阅指定 UP 主的动态。可以添加多个过滤器（以空格分隔）以排除不感兴趣的内容。订阅成功后返回一张展示 UP 主信息、「订阅成功」标题与过滤规则（过滤类型[英文]/过滤正则/直播@全体/推送@全体/图片转发）的卡片图片（不带链接）。再次订阅同一 UP 主更新过滤条件时，同样返回卡片图片，标题为「订阅更新」。 | `订阅动态` |
| **bili_sub_list** | (无) | 以卡片图片形式显示当前会话的所有订阅（含 UID、会话类型 Group/Private、过滤条件等），会话区块同时显示「图片转发」开关状态（若被全局强制则附加标注）。 | `订阅列表` |
...
| **bili_img_forward** | `on\|off\|status` | 开关**当前会话**：多图动态推送时是否以合并消息附带原图（默认关闭）。状态会显示在订阅列表与订阅成功/更新卡片中。 | `图片转发` |
| **bili_img_forward_global** | `on\|off\|clear` | **[管理员]** `on`/`off` 强制所有会话开启/关闭多图原图转发；`clear` 取消强制，恢复各会话自行设置。全局强制优先于会话设置。 | `全局图片转发` |
