...
| 指令 | 参数 | 说明 | 别名 |
| :--- | :--- | :--- | :--- |
| **bili_sub** | `<B站UID> [过滤器...]` | 订阅指定 UP 主的动态。可以添加多个过滤器（以空格分隔）以排除不感兴趣的内容。订阅成功后返回一张展示 UP 主信息、「订阅成功」标题与过滤规则（过滤类型/过滤正则/直播@全体/@全体成员/图片转发）的卡片图片（不带链接）。再次订阅同一 UP 主更新过滤条件时，同样返回卡片图片，标题为「订阅更新」。 | `订阅动态` |
