...
## ✨ 功能特性
  ...
  - **UP 主动态订阅**：
   ...
   - **自然语言视频/番剧搜索**
      - 对 LLM 说 `帮我找一个XXX视频` 返回单个视频图文；`帮我找下XXX相关的视频` 返回视频列表图。
...
| 指令 | 参数 | 说明 | 别名 |
| :--- | :--- | :--- | :--- |
| **bili_sub** | `<B站UID> [过滤器...]` | 订阅指定 UP 主的动态。可以添加多个过滤器（以空格分隔）以排除不感兴趣的内容。订阅成功后返回一张仅展示 UP 主信息与「订阅成功」的卡片图片（不带链接）。 | `订阅动态` |
| **bili_sub_list** | (无) | 以卡片图片形式显示当前会话的所有订阅（含 UID、会话类型 Group/Private、过滤条件等）。 | `订阅列表` |
